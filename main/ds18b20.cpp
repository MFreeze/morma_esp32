...
discoverDsSensors ()
{
    ...
    while (nb_discovered != DS18B20_MEASURES && cur_try < DS18B20_MAX_DISCOVER_TRY)
    {
        ...
        while (ds.search (ds_sensors[nb_discovered].addr))
        {
            ...
            {
                SENSOR_LOGW (DS_NAME, "Bad reception of address for OneWire component %d.\n", nb_discovered + 1);
                continue;
            }
        ...
        cur_try++;
        SENSOR_LOGI (DS_NAME, "End of try %d\n", cur_try);
    }

    SENSOR_LOGI (DS_NAME, "Found %d devices.\n", nb_discovered);
...
readOneWireTemp ()
{
    ...
    for (ds18b20_sensor_t *cur = ds_sensors; cur != ds_sensors + nb_discovered; cur++)
    {
        ...
        SENSOR_LOGI (DS_NAME, "[" DS_ADDR_FORMAT "] Read temperature: %.3f.\n", DS_ADDR_ARGS (cur->addr), cur->t);
    }
    ...
    *(str_parser - 1) = STR_SEP;
    ...
    for (cur = ds_sensors; cur != ds_sensors + nb_discovered; cur ++)
    {
        ...
        currently_printed_chars = snprintf (str_parser,
                                            remaining_size,
                                            "%#04X%02X%02X%02X%02X%02X%02X%02X_t:%.3f%c",
                                            cur->addr[0],
                                            cur->addr[1],
                                            cur->addr[2],
                                            cur->addr[3],
